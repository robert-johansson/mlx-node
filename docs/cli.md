...
- **Sampling is a native-parity reimplementation** (same greedy epsilon, filter order, penalty semantics), and the prompt render is the SAME Rust `applyChatTemplate` — and temp-0 transcript parity vs the `mlx` provider is **measured, not aspirational** (2026-07-17, qwen3.6-35b-a3b-4bit on Thor/CUDA, 5 headless prompts incl. a tool round-trip and a thinking turn, same cwd/day both arms): final stdout byte-identical on all 5; per-turn `usage` (prompt/output/reasoning/cacheRead) exactly equal on all 6 turns, including the tool continuation (854-token re-render, 831 cached, both arms) and the reasoning subset (31 both). Perf, same instrument: every identical-workload invocation completed faster on the genmlx provider (e.g. the 171-token-output turn 37.7s vs 40.3s end-to-end); decode ~11.3 vs ~12.0 tok/s (−6%, within the 20% sanity bound); turn-measured owned prefill ~115 tok/s at 1.5k-token prompts, delta-prefill continuation 3.2s vs 13.1s cold.

Requirements and limits:

- **Families**: qwen3 / qwen3_5 / qwen3_5_moe checkpoints only (what the owned forward implements). **Images work on VLM checkpoints** (genmlx-5aah): attached images (`-p @img.png "…"` — the `@file` must be its own argv token) and tool-result images both route through the owned VLM prefill into the branch ledger — the vision tower runs once per image-set change, and later text-only turns delta-prefill over the image-conditioned branch. Verified live on the 0.8b VLM snapshot and the 35B MoE (leak-controlled: a neutral-named drawing correctly identified; a resumed `-c` session answered the image's background color — a fact stated nowhere in text). Non-VLM checkpoints reject image turns with a typed error.
- **Per-argument grammar constraints** (genmlx-3g0t): a JSON-Schema `pattern` regex on a string tool parameter (TypeBox: `Type.String({ pattern: '…' })`) compiles the declared toolset into a tool-call DFA applied as a per-decode logit mask — malformed tool-call blocks and off-pattern argument values become unrepresentable at sampling time, deleting typecheck-and-retry loops. Activation is annotation-presence (an unannotated toolset decodes exactly as before); the mask sees only visible text (think blocks bypass it); constraint compilation is cached per toolset. `examples/grammar-tool.ts` is the in-tree template. Measured (0.8b, temp 1.0, hostile prompt requesting 6-digit coordinates): unconstrained control emits `999999,888888`, the constrained arm emits `999,888` — and 100 constrained emissions produced zero unparseable calls. The regex dialect is the grammar engine's (printable ASCII, char classes, alternation, `{m,n}`; no lookahead). `x-genmlx-grammar: "cljs"` (reader-level ClojureScript values) is reserved — declaring it is a typed error until it lands. On the v1 `mlx` provider the annotation is inert.
- **Best-of-K with an external verifier** (genmlx-maww, seam contract v1): `MLX_AGENT_BEST_OF_K=<K>` decodes K candidate turns in ONE batched forward over a fork of the session branch, calls the registered verifier once with all K candidates (`{"candidates": [{index, text, finishReason, toolCalls, toolCallErrors}, …]}` → `{"winner": i}` or `{"scores": [K numbers]}`, argmax with ties to the lowest index), and streams the winner as the turn's reply. Extensions register via `globalThis.__GENMLX_TOOL_VERIFIER__` (extension files can only import `@earendil-works/*`); in-process callers use `setGenmlxToolVerifier`/`setGenmlxBestOfK` from `@mlx-node/agent`; `examples/best-of-k-verifier.ts` is the template. Degenerate semantics: verifier absent/throwing/timing-out (`MLX_AGENT_VERIFIER_TIMEOUT_MS`, default 10000) or malformed → candidate 0; K=1/unset → the scalar path unchanged. Lockstep parity is test-pinned (a temp-0 K-turn reproduces the scalar greedy text byte-for-byte); the session commit stays honest (prompt-only — the next turn delta-prefills the winner's re-rendered reply, verified live: 806/854 cached on the tool continuation). v1 scope: no thinking or per-argument-grammar composition with K>1 (typed errors); candidate turns don't stream token-by-token (the winner arrives as one delta).
