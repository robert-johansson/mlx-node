...
| 11 | **The Qwen pre-stacked-expert branch drops tensors it does not recognize.** The key list is built with `k.contains(".experts.gate_up_proj")`, every match is `remove()`d, but only `k.ends_with(…)` re-inserts. `…experts.gate_up_proj.weight` / `.scales` / `.biases` are removed and never written. | `crates/mlx-core/src/convert.rs:967`, `:981`                                     |
...
| 17 | **`audio_tower.*` / `audio_encoder.*` are quantized on the way out and unconditionally discarded on the way in.** `Gemma4Recipe::sanitize` keeps them, `should_quantize` has no audio arm, so `--quantize` writes packed weights + `.scales` for them; the loader skips every such key outright. The audio exclusion at `convert.rs:6252` fires **only** when `default_mode == "sym8"`. | `crates/mlx-core/src/convert.rs:6259`, `crates/mlx-core/src/models/gemma4/persistence.rs:855` |
...
| 41 | **`mlx convert -m paddleocr-vl -q` / `-m qianfan-ocr -q` is accepted** and emits a checkpoint neither loader can read — both model modules contain zero quantization code, yet both recipes leave `quant_managed_by_sanitizer()` at `false`, so the generic quantizer writes `.scales`/`.biases` for them. The CLI has an explicit `--quantize` reject for `pp-lcnet-ori`/`uvdoc` one branch away. | `crates/mlx-core/src/convert.rs:1520`, `:1540`, `packages/cli/src/commands/convert.ts:238` |
...
| claim                                                                                                                                        | where                                                | reality                                                                                                                                                                                       |
| --------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| "exposes **three** top-level commands"                                                                                                       | `docs/cli.md:3`                                      | Six: `download`, `convert`, `calibrate`, `redact`, `launch`, `agent` (`packages/cli/src/cli.ts:52`). The same file documents `calibrate` and `agent` further down.                             |
