...
impl MxArray {
    ...
    // The `shapeArray` accessor (shape as JS number[] for ClojureScript/nbb) was
    // relocated to `genmlx-core/src/genmlx.rs` as the `shapeOf` free fn calling
    // the same mlx-sys FFI directly (W-B) — mlx-core is stock here.
