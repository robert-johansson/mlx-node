...
impl Qwen3_5Attention {
    ...
    pub fn forward(
        ...
        let (queries, keys) = if let (Some(pos_ids), Some(mrope)) = (position_ids, &self.mrope) {
            ...
            // Text-only scalar RoPE is DEFERRED to AFTER the [B, H, T, D]
            // transpose below (axis -2 = sequence), where it is applied
            // exactly once (mlx-cft4; equivalent to upstream 4c65f80's
            // transpose-in/rotate/transpose-back without the extra views).
            (queries, keys)
        };
    ...
    pub fn forward_paged(
        ...
        let (queries, keys) = if let (Some(pos_ids), Some(mrope)) = (position_ids, &self.mrope) {
            ...
            // Text-only scalar RoPE is DEFERRED to AFTER the transpose below
            // (axis -2 = SEQUENCE), where it uses `rope_position_offset` —
            // which decouples the rotation position from the physical KV
            // slot: a turn that warm-continues an image prefill rotates at
            // the compressed M-RoPE position while K/V still writes at the
            // physical slot. (mlx-cft4)
            (queries, keys)
        };
