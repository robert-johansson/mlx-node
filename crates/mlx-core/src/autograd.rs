...
// The general-purpose JS-callback `valueAndGrad` / `computeGradients` NAPI exports
// were relocated to `genmlx-core/src/genmlx.rs` (W-B): the orphan rule forbids an
// `impl MxArray` outside mlx-core, so there they are module-level free fns wrapping
// the generic `value_and_grad` / `compute_gradients` above. mlx-core keeps only the
